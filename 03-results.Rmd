...
cat5_vc_df <- df1 %>%
  ...
  select(`final triage category`, `call outcome`,`clinician category`, TimeCallCommenced)
...
An analysis of catgory 5 calls managed by the clinical support desk show that a higher proportion of calls that were intially triaged as category 5 remained so when a video triage was conducted. Similarly, a high proportion of calls had an outcome of 'hear and treat' compared to traditional telephone-only triage (Table \@ref(tab:cat-table) and Figure \@ref(fig:csd-vs-vc)).
...
```{r csd-vs-vc, fig.cap="Comparison of clinical support desk and video triage category 5 calls"}
  ...
  cat5_df2 %>%
  pivot_longer(cols = c(prop_ht, prop_c5), names_to = "variable", values_to = "proportion") %>%
  mutate(
    variable = ifelse(variable == "prop_ht", "Proportion of hear and treat call outcome", "Proportion of calls with a final triage category of 5")
  ) %>%
