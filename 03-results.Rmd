...
createPlot <- function(dataframe,thelegend=FALSE, wrapwidth=40) {
  ...
  plot <- plot(likert(dataframe), type="bar", wrap=wrapwidth, include.center=T, plot.percents=F,plot.percent.low=T, plot.percent.high=T, plot.percent.neutral=T, legend.position="bottom", colors=c('#CC0016','#F6A57F','#DFDFDF','#90C5DF','#0070B2')) 
  return(plot)
}
...
df1 %>%
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, size = 10), axis.title.x = element_text(vjust = -2), plot.margin = unit(c(1,1,1,1), "cm"), legend.position = "bottom")
