...
df1 <- df %>%
  ...
  mutate(
    ...
    "Hear and treat recontact within 24 hours" = recontact
  )
...
summary_table <- CreateTableOne(
  vars = c("month", "CBU","initial triage category", "final triage category", "age", "sex", "video call conducted?", "technical issue during call?", "AMPDS chief complaint", "call outcome"),
  strata = "video",
  data = df1, 
  test = F,
  factorVars = c("CBU", "technical issue during call?", "initial triage category", "final triage category", "AMPDS chief complaint", "sex", "month", "video call conducted?", "call outcome"),
  addOverall = T,
  includeNA = T
)
...
An analysis of catgory 5 calls show that a higher proportion of calls that were intially triaged as category 5 remained so when a video triage was conducted compared to traditional telephone triage conducted by the clinical support desk. Similarly, a higher proportion of calls had an outcome of 'hear and treat' when video triage was utilised (Table \@ref(tab:cat-table) and Figure \@ref(fig:csd-vs-vc)). Recontact rates for the subset of calls with a 'hear and treat' outcome were `r r24a$n[r24a$cc == "csd" & r24a$recontact == "yes"]`/`r r24$n[r24$cc == "csd"]` (`r round((r24a$n[r24a$cc == "csd" & r24a$recontact == "yes"]/ r24$n[r24$cc == "csd"])*100,1)`%) for clinical support desk calls and `r r24a$n[r24a$cc == "video" & r24a$recontact == "yes"]`/`r r24$n[r24$cc == "video"]` (`r round((r24a$n[r24a$cc == "video" & r24a$recontact == "yes"]/ r24$n[r24$cc == "video"])*100,1)`%) for the video calls.
...
```{r csd-vs-vc, fig.cap="Comparison of clinical support desk and video triage category 5 calls"}
  ...
  mutate(
    variable = case_when(
      variable == "prop_ht"~  "Proportion of hear and treat call outcome", 
      variable == "prop_c5" ~ "Proportion of calls with a final triage category of 5",
      TRUE ~ "Proportion of hear and treat calls\nwith a recontact within 24 hours")
  ) %>%
